...
SECRET_PATTERNS = [
    ...
    (re.compile(r"(?i)(bearer\s+)[A-Za-z0-9._~+/=-]{20,}"), r"\1[REDACTED_SECRET]"),
    # Common API token/key/password assignments in logs, prompts, and shell snippets.
    (
        re.compile(
            r"(?i)\b([A-Z0-9_]*(?:API[_-]?KEY|TOKEN|SECRET|PASSWORD|PASSWD|PRIVATE[_-]?KEY)[A-Z0-9_]*\s*[:=]\s*)"
            r"([\"']?)[^\s\"']{8,}\2"
        ),
        r"\1[REDACTED_SECRET]",
    ),
    (
        re.compile(
            r"(?i)\b(export\s+[A-Z0-9_]*(?:API[_-]?KEY|TOKEN|SECRET|PASSWORD|PASSWD|PRIVATE[_-]?KEY)[A-Z0-9_]*\s*=\s*)"
            r"([\"']?)[^\s\"']{8,}\2"
        ),
        r"\1[REDACTED_SECRET]",
    ),
    # Well-known token prefixes.
    (re.compile(r"\b(sk-[A-Za-z0-9_-]{16,})\b"), "[REDACTED_SECRET]"),
    (re.compile(r"\b(hsk_[A-Za-z0-9_-]{16,})\b"), "[REDACTED_SECRET]"),
    (re.compile(r"\b(ghp_[A-Za-z0-9_]{16,})\b"), "[REDACTED_SECRET]"),
    (re.compile(r"\b(github_pat_[A-Za-z0-9_]{16,})\b"), "[REDACTED_SECRET]"),
    # URLs with embedded credentials.
    (re.compile(r"([a-zA-Z][a-zA-Z0-9+.-]*://)[^/\s:@]+:[^/\s@]+@"), r"\1[REDACTED_SECRET]@"),
]


def redact_secrets(content: str) -> str:
    """Best-effort local redaction before sending transcript content to memory."""
...
def redact_secrets_deep(value):
    """Recursively redact strings inside structured transcript blocks."""
    if isinstance(value, str):
        return redact_secrets(value)
    if isinstance(value, list):
        return [redact_secrets_deep(item) for item in value]
    if isinstance(value, dict):
        return {key: redact_secrets_deep(item) for key, item in value.items()}
    return value
...
def _is_synthetic_claude_text(text: str) -> bool:
    """Return True for harness-injected text Claude Code persists as user chat."""
    return text.lstrip().startswith(CLAUDE_SYNTHETIC_PREFIXES)


def _claude_text_from_content(content) -> str:
    """Extract plain text from a Claude Code message/tool_result content value.

    Accepts a raw string or a list of content blocks; only "text" blocks are
    kept and synthetic harness-injected blocks are dropped.
    """
    if isinstance(content, str):
        text = content.strip()
        return "" if _is_synthetic_claude_text(text) else text
    if isinstance(content, list):
        parts = []
        for block in content:
            if isinstance(block, dict) and block.get("type") == "text":
                t = block.get("text", "").strip()
                if t and not _is_synthetic_claude_text(t):
                    parts.append(t)
        return "\n".join(parts).strip()
    return ""


def _is_claude_chat_entry(entry: dict) -> bool:
    """Return True for a Claude Code transcript chat line (user/assistant)."""
    return (
        ...
        with open(transcript_path, encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    elif _is_claude_chat_entry(entry):
                        msg = entry["message"]
                        role = msg.get("role", "")
                        if role not in ("user", "assistant"):
                            continue
    ...
    try:
        with open(transcript_path, encoding="utf-8") as f:
            for line in f:
                ...
                # Claude Code transcript format
                if _is_claude_chat_entry(entry):
                    msg = entry["message"]
                    role = msg.get("role", "")
                    ...
                    if role == "user":
                        ...
                        for block in content:
                            ...
                            if btype == "text":
                                ...
                                # Claude Code delivers tool results as user-role
                                # entries; group them with the assistant turn.
                                out = _claude_text_from_content(block.get("content"))
                                if out:
...
def _prepare_json_transcript(messages: list, allowed_roles: set) -> tuple:
    ...
    for msg in messages:
        ...
        structured_messages.append({"role": role, "content": redact_secrets_deep(blocks)})
...
def _prepare_text_transcript(messages: list, allowed_roles: set) -> tuple:
    ...
    for msg in messages:
        ...
        if not isinstance(content, str):
            content = str(content)
        content = redact_secrets(strip_memory_tags(content)).strip()
