...
def ensure_pillow():
    if importlib.util.find_spec("PIL") is not None:
        print("ok: Pillow", flush=True)
        return
    if not command_exists("pip3"):
        raise SystemExit("pip3 is required to install Pillow.")
    run(["pip3", "install", "--user", "Pillow"])
    if importlib.util.find_spec("PIL") is None:
        raise SystemExit("Pillow installation did not become importable in this Python.")
...
def main():
    ...
    print("\nvideo-highlight dependencies are ready.")
