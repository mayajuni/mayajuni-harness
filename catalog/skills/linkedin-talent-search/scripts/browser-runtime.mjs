...
const TASK_TAB_PREFIX = "linkedin-talent";

const command = process.argv[2] ?? "help";
const commandArgs = process.argv.slice(3);
const port = parsePort(process.env.LINKEDIN_TALENT_CDP_PORT ?? DEFAULT_PORT);
const profileDir = path.resolve(
  process.env.LINKEDIN_TALENT_PROFILE_DIR ?? DEFAULT_PROFILE_DIR,
);
const agentBrowserVersion =
  process.env.LINKEDIN_TALENT_AGENT_BROWSER_VERSION ??
  DEFAULT_AGENT_BROWSER_VERSION;
const closeOnComplete = parseClosePolicy(
  process.env.LINKEDIN_TALENT_CLOSE_ON_COMPLETE ?? DEFAULT_CLOSE_ON_COMPLETE,
);
...
function printHelp() {
  process.stdout.write(`LinkedIn Talent Search browser runtime\n\n`);
  ...
  process.stdout.write(`  LINKEDIN_TALENT_CDP_PORT (default: ${DEFAULT_PORT})\n`);
  process.stdout.write(`  LINKEDIN_TALENT_PROFILE_DIR (default: ${DEFAULT_PROFILE_DIR})\n`);
  process.stdout.write(
    `  LINKEDIN_TALENT_AGENT_BROWSER_VERSION (default: ${DEFAULT_AGENT_BROWSER_VERSION})\n`,
  );
  process.stdout.write(
    `  LINKEDIN_TALENT_CLOSE_ON_COMPLETE (tab|none, default: ${DEFAULT_CLOSE_ON_COMPLETE})\n`,
  );
}
...
async function ensureBrowser() {
  ...
  if (existing) {
    printJson({
      ...
      note: "Reused the existing dedicated CDP endpoint.",
    });
    return;
  }
    ...
    failJson(
      {
        ...
        locks,
        message:
          "The dedicated profile is locked while its CDP endpoint is unavailable. Close or recover that browser manually; lock files were not changed.",
      },
      2,
    );
  }
  ...
  failJson(
    {
      ...
      endpoint: endpointFor(port),
      profileDir,
      message: "Chrome did not expose the dedicated CDP endpoint within 15 seconds.",
    },
    2,
  );
}
...
function launchChrome() {
  ...
  if (process.platform === "darwin") {
    const appName = process.env.LINKEDIN_TALENT_CHROME_APP ?? "Google Chrome Beta";
    detach("/usr/bin/open", ["-na", appName, "--args", ...chromeArgs]);
    return;
  }
  ...
  if (process.platform === "win32") {
    ...
    if (!executable) {
      fail("Google Chrome Beta or Chrome was not found on Windows.", 2);
    }
    detach(executable, chromeArgs);
    return;
  }
  ...
  const executable =
    ...
    findOnPath("chromium");
  if (!executable) {
    fail("Google Chrome Beta, Chrome, or Chromium was not found on PATH.", 2);
  }
  detach(executable, chromeArgs);
}
...
function invokeAgentBrowser(args) {
  ...
  return spawnSync(npm, npmArgs, { stdio: "inherit" });
}
...
function findWindowsChrome() {
  const candidates = [
    process.env.LINKEDIN_TALENT_CHROME_PATH,
...
function parsePort(value) {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 65535) {
    fail(`Invalid LINKEDIN_TALENT_CDP_PORT: ${value}`, 1);
  }
  return parsed;
}
...
function parseClosePolicy(value) {
  if (value !== "tab" && value !== "none") {
    fail(`Invalid LINKEDIN_TALENT_CLOSE_ON_COMPLETE: ${value}`, 1);
  }
  return value;
}
